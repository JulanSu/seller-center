...
<script>
  ...
  import { 
    ...
    getStoreCate, 
    theShelvestoSoldOut, 
    onTheShelvesRecommendOff,
    onTheShelvesRecommendOn } from '@/api/seller'
  ...
  const storeId = win.config && win.config.storeId ? win.config.storeId : ''
    export default {
        ...
        methods: {
          ...
          /**
           * onRecommendOnHandle 推荐/取消推荐回调方法
           * @param  { Object } row 当前行的数据
           * @return {[type]}     [description]
           */
          onRecommendOnHandle (row) {
            let self = this
            if(!row.productRecommend){
              self.confirmHandle({
                title: '商品推荐',
                content: '确定要推荐这个商品吗？'
              }, function(){
                onTheShelvesRecommendOn({
                  productId: row.productId
                }).then((res)=>{
                  self.listLoading = false;
           ...
           * @return {[type]}     [description]
           */
          onRecommendOffHandle (row) {
            let self = this
            if(row.productRecommend){
              self.confirmHandle({
                title: '商品取消推荐',
                content: '确定要取消推荐这个商品吗？'
              }, function(){
                onTheShelvesRecommendOff({
                  productId: row.productId
                }).then((res)=>{
          ...
          /**
           * searchSubmitHandle 查询商品回调方法
           * @param  { Object } value 用户输入搜索条件的对象
           * @return {[type]}       [description]
           */
