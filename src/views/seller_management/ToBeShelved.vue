    ...
    <search-nav @onSearchClick="searchSubmitHandle"></search-nav>
      <el-table
        ...
        <el-table-column
          ...
          label="操作">
          <template slot-scope="scope">
            <el-button type="text" @click="onShelvesReadyToOnHandle(scope.row)">立即上架</el-button>
            <el-button type="text" @click="onReadyToShelvesToSoldOutHandle(scope.row)">取消上架</el-button>
          </template>
        </el-table-column>
      </el-table>
...
<script>
  ...
  import { getProductList, getStoreCate, onShelvesReadyToOn, onReadyToShelvesToSoldOut} from '@/api/seller'
  ...
  const storeId = win.config && win.config.storeId ? win.config.storeId : ''
    export default {
        ...
        methods: {
          onShelvesReadyToOnHandle (row){
            let self = this
            self.confirmHandle({
              title: '立即上架',
              content: '确定要立即上架这个商品吗'
            }, function(){
              onShelvesReadyToOn({
          ...
          onReadyToShelvesToSoldOutHandle (row) {
            let self = this
            self.confirmHandle({
              title: '取消上架',
              content: '确定要取消上架这个商品吗'
            }, function(){
              onReadyToShelvesToSoldOut({
                productId: row.productId
              }).then((res)=>{
                var data = res.data.data
                if(res.data.code == 0) {
